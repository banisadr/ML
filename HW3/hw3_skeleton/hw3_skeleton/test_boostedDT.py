...
#SVC_pred = linearSVC.predict(Xtrain)
linearSVC = svm.SVC()
linearSVC.fit(Xtrain,ytrain)
SVC_pred = linearSVC.predict(Xtrain)


accuracyChallenge = accuracy_score(ytrain, SVC_pred)
print "Best accuracy for the challenge training data set is " + str(accuracyChallenge)
challengePred = newBoosted.predict(testDataset)
predictions = ""
for i in range(0, challengePred.size):
    predictions += str(int(challengePred[i])) + ","  
